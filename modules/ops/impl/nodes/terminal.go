...
func (n *Nodes) Terminal(w http.ResponseWriter, r *http.Request) {
		...
		if ForwardPort > 0 && a.Name == "docker" { //
			...
			go func() {
				...
				for {
					t, b, err := conn.ReadMessage()
					if err != nil {
						logrus.Debugf("read message error: %v", err)
						return
					}
					err = c.WriteMessage(t, b)
					if err != nil {
						logrus.Debugf("write message error: %v", err)
						return
					}
				}
			}()
	...
	if err != nil {
		logrus.Debugf("terminal context error: %v", err)
		return
	}
	slave, err := pty.Start(cmd)
	if err != nil {
		logrus.Debugf("pty start error: %v", err)
		return
	}
	defer slave.Close()
	...
	go func() {
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					logrus.Debugf("salve read error: %v", err)
				}
				if n > 0 {
					err = write(b[:n+1])
					if err != nil {
						logrus.Debugf("slave write: %v", err)
					}
				}
				return
			}
	...
	go func() {
		...
		for {
			t, b, err := conn.ReadMessage()
			if err != nil {
				logrus.Debugf("connection read message: %v", err)
				return
			}
			...
			case Ping:
				...
				err := write([]byte{Pong})
				if err != nil {
					logrus.Debugf("case Ping write error: %v", err)
					return
				}
			case Pong:
				if len(b) != 1 {
			...
			case Input:
				...
				if err != nil {
					logrus.Debugf("case Input write error: %v", err)
					return
				}
			case GetSize:
				if len(b) != 1 {
				...
				if err != nil {
					logrus.Debugf("case GetSize full error: %v", err)
					return
				}
				b, err := json.Marshal(v)
				if err != nil {
					logrus.Debugf("case GetSize marshal error: %v", err)
					return
				}
				err = write(append([]byte{Size}, b...))
				if err != nil {
					logrus.Debugf("case GetSize write error: %v", err)
					return
				}
			case SetSize:
				var v pty.Winsize
				...
				if err != nil {
					logrus.Debugf("case SetSize unmarshal error: %v", err)
					return
				}
				err = pty.Setsize(slave, &v)
				if err != nil {
					logrus.Debugf("case SetSize pty error: %v", err)
					return
				}
			default:
				return
	...
	for {
		...
		case <-t:
			if err := write([]byte{Ping}); err != nil {
				logrus.Debugf("time out error: %v", err)
				return
			}
		}
	}
